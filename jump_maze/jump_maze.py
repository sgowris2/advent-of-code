...
def find_exit_steps(jumps):

    pos = 0
    steps = 0
    while len(jumps) > pos >= 0:
...
if __name__ == '__main__':
    ...
    # print(find_exit_steps([0, 3, 0, 1, -3]))
    # print(find_exit_steps_2([0, 3, 0, 1, -3]))
    with open('input.txt') as f:
        content = f.readlines()
        _jumps = [int(x.strip()) for x in content]
        # print(find_exit_steps(_jumps))
        print(find_exit_steps_2(_jumps))
...
# The message includes a list of the offsets for each jump. Jumps are relative: -1 moves to the previous instruction,
        #  and 2 skips the next one. Start at the first instruction in the list. The goal is to follow the jumps
...
# In addition, these instructions are a little strange; after each jump, the offset of that instruction increases by
        # 1. So, if you come across an offset of 3, you would move three instructions forward, but change it to a 4
        # for the next time it is encountered.
#
# For example, consider the following list of jump offsets:
#
# 0 3 0 1 -3 Positive jumps ("forward") move downward; negative jumps move upward. For legibility in this example,
